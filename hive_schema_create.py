...
class HiveSchemaCreate:
    ...
    def get_field_key_info(self):
        ...
        """
        ret = re.search(re_select_from, self.hive_sql, re.S | re.M | re.I)  # match 从开头开始匹配
        field_str = ret.groups()[1]
        ...
        for field_info in field_arr:
            ...
            try:
                ...
                field_info_tuple = re.findall(re_field_info, field_info)[0]
                field_name = field_info_tuple[0]
                ...
                try:
                    re_field_info_not_type = re.compile(r'([\s\S][^-]*)([-]+)(.*)', re.S)  # 最小匹配并且用户未定义字段类型的
                    field_info_tuple = re.findall(re_field_info_not_type, field_info)[0]
                    field_name = field_info_tuple[0]
                    if field_name.find(',') == 0:
                        field_name = field_name[field_name.find(',') + 1:]
                    field_type = "string"
    ...
    def hive_sql_to_hive_schema(self):
        ...
        for field_index, key_info in enumerate(field_key_info_arr):
            ...
            # print field_comment
            schema_line = "    {}{} {} comment '{}'".format(' ' if field_index == 0 else ',',
                                                         field_name.strip(),
                                                         field_type.strip(),
                                                         field_comment.strip()
                                                           )
        ...
        schema_ret += '\n'
        schema_head = "create external table if not exists 'Hive表格名称{}' (".format(self.table_name)
        ...
        schema_tail = """
          ...
          stored as orc 
          location '/user/bigdata/Hive所属层{}/Hive表名称{}  ' 
    ...
    def hive_sql_to_mysql_scheme(self):
        ...
        mysql_head = "CREATE TABLE {} (".format('输入mysql表格英文名')
...
if __name__ == '__main__':
    ...
    with open('./sample2.sql', 'r') as f:
        hive_sql = f.read().strip()


    #
    #
    hive_util = HiveSchemaCreate(hive_sql)
    # #
    # sys.exit(0)
